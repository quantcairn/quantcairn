...
def _roots(tmp_path: Path, name: str) -> dict[str, Path]:
    root = tmp_path / name
    return {key: root / key for key in ("project", "state", "reports", "artifacts", "logs")}
...
def test_runtime_resolver_supports_import_before_env_and_a_to_b_switch(tmp_path, monkeypatch):
    roots_a, roots_b = _roots(tmp_path, "a"), _roots(tmp_path, "b")
...
def test_wrapper_log_state_and_lock_paths_are_operation_time_resolved(tmp_path, monkeypatch):
    ...
    assert module._runtime_lock_file().parent == roots_a["state"].resolve()
    _set_roots(monkeypatch, roots_b)
    assert module._runtime_log_paths()[0].parent == roots_b["logs"].resolve()
    assert module._runtime_state_dir() == roots_b["state"].resolve()
    assert module._runtime_lock_file().parent == roots_b["state"].resolve()


def test_preflight_universe_bundle_and_refinement_switch_roots(tmp_path, monkeypatch):
    ...
    bundle = bundle_module.SelectionBundle(
        summary={}, selection_state_payload={}, top_items=[], selection_run_id="run",
        selection_date="2026-01-01", generated_at="now", processing_phase="",
        selection_stage="", result_quality="", research_admission="",
    )
...
def test_selector_runner_report_and_log_consumers_switch_roots(tmp_path, monkeypatch):
...
def test_runtime_paths_are_cwd_independent_when_unset(tmp_path, monkeypatch):
