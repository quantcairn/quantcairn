...
def test_release_tag_version_normalization():
    module = _load_module()
    assert module._normalize_release_tag_version("v0.12.14-dashboard-performance-optimization") == "0.12.14"
    assert module._normalize_release_tag_version("refs/tags/v0.12.14-dashboard-performance-optimization") == "0.12.14"
    assert module._normalize_release_tag_version("0.12.14") == "0.12.14"
...
def test_release_version_matches_pyproject_and_package(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(module, "_read_package_version", lambda: "0.12.14")
    result = module.check_release_version(tag="v0.12.14-dashboard-performance-optimization", project_root=project_root)

    assert result["pyproject_version"] == "0.12.14"
    assert result["package_version"] == "0.12.14"
    assert result["release_tag_version"] == "0.12.14"
...
def test_release_tag_version_mismatch_fails_fast(tmp_path: Path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="release tag version mismatch"):
        module.check_release_version(tag="v0.12.15-dashboard-performance-optimization", project_root=project_root)
...
def test_main_reports_success_and_failure(tmp_path: Path, monkeypatch, capsys):
    ...
    exit_code = module.main(["--tag", "v0.12.14-dashboard-performance-optimization", "--project-root", str(project_root)])
